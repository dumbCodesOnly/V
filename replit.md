...
- **TP Allocation Calculation Fix (2025-08-19)**: Corrected partial take profit allocation calculations. Fixed the profit amount calculation to properly account for position sizing in futures trading. Now correctly calculates total position size based on margin and leverage, then applies allocation percentages to determine the actual position size closed at each TP level. Profit amounts are now calculated as price_difference × position_size_closed, providing accurate P&L calculations for partial position closures.
