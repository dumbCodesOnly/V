...
**Enhanced TP/SL Display (Aug 2025)**: Implemented comprehensive take profit and stop loss enhancement showing actual prices alongside profit/loss amounts instead of just percentages. Added `calculate_tp_sl_prices_and_amounts()` function that computes exact TP/SL trigger prices and expected profit/loss amounts based on position size and entry price. Fixed profit/loss calculation logic to use contract-based calculations (amount * leverage / entry_price) for accurate P&L amounts. Updated both web interface and Telegram bot to display: TP levels with exact prices and profit amounts (e.g., "TP1: $45,200 (+$125.50)"), SL with exact price and loss amount (e.g., "SL: $42,800 (-$220.00)"). Enhanced user experience by showing real financial impact of risk management levels rather than abstract percentages, helping users make more informed trading decisions.
