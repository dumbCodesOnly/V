...
class ExchangeSyncService:
    ...
    def _sync_loop(self):
        ...
        while self.running:
            try:
                ...
                is_boost_active = (
                    (self.is_render or not self.is_vercel)
                    and self.last_health_ping
                    and (datetime.utcnow() - self.last_health_ping).total_seconds()
                    <= TimeConfig.HEALTH_PING_BOOST_DURATION
                )
    ...
    def _get_current_sync_interval(self):
        ...
        time_since_health_ping = (
            datetime.utcnow() - self.last_health_ping
        ).total_seconds()
    ...
    def _sync_all_users(self):
        ...
        with self.app.app_context():
            try:
                ...
                logging.info(
                    f"SYNC: Found {len(users_with_creds)} users with active credentials"
                )
                ...
                for user_creds in users_with_creds:
                    ...
                    if active_trades:
                        logging.info(
                            f"SYNC: Syncing {len(active_trades)} active trades for user {user_id}"
                        )
                        self._sync_user_positions(user_creds)
                        users_synced += 1
                    else:
                        logging.debug(
                            f"SYNC: User {user_id} has no active trades to sync"
                        )

                logging.info(
                    f"SYNC: Completed real trading sync for {users_synced} users"
                )
    ...
    def _quick_user_check(self):
        ...
        try:
            with self.app.app_context():
                ...
                if active_count > 0:
                    logging.info(
                        f"COST OPTIMIZATION: Found {active_count} active users - will resume full sync"
                    )
        except Exception as e:
            logging.debug(f"Quick user check error: {e}")

    def _debug_position_status(self, trade_id, config):
        """Debug log position status for diagnostics."""
        if config.status == "active":
            ...
            logging.info(
                f"DEBUGGING Position {trade_id}: paper_mode={paper_mode_flag}, "
    ...
    def _should_monitor_paper_position(self, config, user_id):
        """Determine if a position should be monitored for paper trading."""
        from api.app import user_paper_trading_preferences

        user_is_paper_mode = user_paper_trading_preferences.get(
            int(user_id), True
        )  # Default paper mode

        return (
            config.status == "active"
            and hasattr(config, "take_profits")
            and config.take_profits
            and (
                getattr(config, "paper_trading_mode", False)  # Explicit paper flag
                or hasattr(config, "paper_tp_levels")  # Has paper TP data
                or hasattr(config, "paper_sl_data")  # Has paper SL data
                or str(getattr(config, "exchange_order_id", "")).startswith(
                    "paper_"
                )  # Paper order ID
                or user_is_paper_mode  # CRITICAL FIX: User is in paper mode, so position should be paper trading
            )
        )

    def _initialize_paper_trading_structures(self, config, trade_id):
        """Initialize missing paper trading monitoring structures."""
        ...
        if not hasattr(config, "paper_tp_levels") and config.take_profits:
            ...
            if tp_sl_data.get("take_profits"):
                ...
                logging.info(
                    f"FIXED: Initialized {len(config.paper_tp_levels)} paper TP levels for position {trade_id}"
                )
            ...
            if tp_sl_data.get("stop_loss") and config.stop_loss_percent > 0:
                ...
                logging.info(
                    f"FIXED: Initialized paper SL data for position {trade_id}"
                )

    def _process_single_paper_position(self, user_id, trade_id, config):
        """Process a single paper trading position."""
        from api.app import get_live_market_price, process_paper_trading_position
    ...
    def _sync_paper_trading_positions(self):
        ...
        try:
            ...
            from api.app import user_trade_configs
            ...
            for user_id, trades in user_trade_configs.items():
                for trade_id, config in trades.items():
                    # Debug logging for active positions
                    self._debug_position_status(trade_id, config)

                    # Check if this position should be monitored for paper trading
                    if self._should_monitor_paper_position(config, user_id):
                        try:
                            # Initialize paper trading structures if missing
                            self._initialize_paper_trading_structures(config, trade_id)

                            # Process the paper trading position
                            if self._process_single_paper_position(
                                user_id, trade_id, config
                            ):
                                paper_positions_processed += 1
                        except Exception as position_error:
                            logging.error(
                                f"Error processing paper position {trade_id}: {position_error}"
                            )

            if paper_positions_processed > 0:
                logging.info(
                    f"PAPER TRADING MONITORING: Processed {paper_positions_processed} paper trading positions"
                )
    ...
    def _sync_user_positions(self, user_creds: UserCredentials):
        ...
        try:
            ...
            manual_paper_mode = user_paper_trading_preferences.get(
                chat_id, True
            )  # Default to paper trading
            is_paper_mode = (
                manual_paper_mode
                or not user_creds
                or user_creds.testnet_mode
            ...
            if is_paper_mode:
                logging.debug(
                    f"Skipping live sync for user {user_id} - in paper trading mode"
                )
                return
            ...
            for trade in local_trades:
                self._sync_individual_trade(
                    trade, exchange_positions, exchange_orders, client
                )
    ...
    def _sync_individual_trade(
        ...
        try:
            ...
            exchange_position = next(
                (pos for pos in exchange_positions if pos.get("symbol") == symbol), None
            )

            # Find related orders
            related_orders = [
                order for order in exchange_orders if order.get("symbol") == symbol
            ]
            ...
            if not exchange_position or float(exchange_position.get("size", 0)) == 0:
                ...
                filled_orders = [
                    order for order in related_orders if order.get("status") == "filled"
                ]

                if filled_orders:
                    # Calculate final P&L from filled orders
                    final_pnl = self._calculate_final_pnl_from_orders(
                        filled_orders, trade
                    )
                    ...
                    logging.info(
                        f"Trade {trade.trade_id} closed on exchange with P&L: {final_pnl}"
                    )

            else:
                # Position still active - update current data
                current_price = float(
                    exchange_position.get("markPrice", trade.current_price)
                )
    ...
    def _calculate_final_pnl_from_orders(
        self, filled_orders: List[Dict], trade: TradeConfiguration
    ) -> float:
        """Calculate final P&L from filled orders"""
        try:
            total_pnl = 0.0
    ...
    def _process_partial_tp_fills(
        self, trade: TradeConfiguration, tp_fills: List[Dict]
    ):
        ...
        try:
            ...
            for tp_fill in tp_fills:
                ...
                fill_quantity = tp_fill[
                    "quantity"
                ]  # Actual position size closed by exchange
            ...
            if not hasattr(trade, "original_margin") or trade.original_margin is None:
                ...
                trade.original_margin = (
                    trade.original_amount / trade.leverage
                    if trade.leverage > 0
                    else trade.original_amount
                )
            ...
            self.db.session.commit()
            logging.info(
                f"Updated realized P&L for trade {trade.trade_id}: ${trade.realized_pnl:.2f} (added ${total_realized_pnl:.2f})"
            )
            logging.info(
                f"Preserved original allocation amounts - Original: ${trade.original_amount:.2f}, Current: ${trade.amount:.2f}"
            )
            ...
            if current_tps:
                ...
                if hasattr(trade, "breakeven_after") and trade.breakeven_after > 0:
                    if not getattr(trade, "breakeven_sl_triggered", False):
                        ...
                        logging.info(
                            f"Breakeven stop loss triggered for trade {trade.trade_id} - SL moved to entry price {trade.entry_price}"
                        )

            logging.info(
                f"Updated TP levels for trade {trade.trade_id}. Remaining TPs: {len(current_tps)}"
            )
