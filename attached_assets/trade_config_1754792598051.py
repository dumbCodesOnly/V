...
class TradeConfig:
    ...
    def validate(self) -> List[str]:
        ...
        # Validate TP size percentages
        tp_errors = self.validate_tp_size_percentages()
        errors.extend(tp_errors)
        
        # Validate price relationships
        if self.entry_price and self.sl_price:
            if self.side == "long" and self.sl_price >= self.entry_price:
                errors.append("Stop loss price must be below entry price for long positions")
            elif self.side == "short" and self.sl_price <= self.entry_price:
                errors.append("Stop loss price must be above entry price for short positions")
        
        # Validate TP price ordering for long positions
        if self.side == "long" and self.entry_price:
            tp_prices = []
            if self.tp1_price:
                tp_prices.append(("TP1", self.tp1_price))
            if self.tp2_price:
                tp_prices.append(("TP2", self.tp2_price))
            if self.tp3_price:
                tp_prices.append(("TP3", self.tp3_price))
            
            for tp_name, tp_price in tp_prices:
                if tp_price <= self.entry_price:
                    errors.append(f"{tp_name} price must be above entry price for long positions")
            
            # Check TP ordering
            if len(tp_prices) >= 2:
                for i in range(len(tp_prices) - 1):
                    if tp_prices[i][1] >= tp_prices[i + 1][1]:
                        errors.append(f"{tp_prices[i][0]} price must be below {tp_prices[i + 1][0]} price")
        
        # Validate TP price ordering for short positions
        elif self.side == "short" and self.entry_price:
            tp_prices = []
            if self.tp1_price:
                tp_prices.append(("TP1", self.tp1_price))
            if self.tp2_price:
            ...
            for tp_name, tp_price in tp_prices:
                if tp_price >= self.entry_price:
                    errors.append(f"{tp_name} price must be below entry price for short positions")
            
            # Check TP ordering
            if len(tp_prices) >= 2:
                for i in range(len(tp_prices) - 1):
                    if tp_prices[i][1] <= tp_prices[i + 1][1]:
                        errors.append(f"{tp_prices[i][0]} price must be above {tp_prices[i + 1][0]} price")
        ...
        if self.trail_activation_percent < 0 or self.trail_activation_percent > 100:
            errors.append("Trailing stop activation percentage must be between 0 and 100")
    ...
    def from_dict(cls, data: dict) -> 'TradeConfig':
        ...
        config = cls(data.get('trade_id'))
        
        # Set datetime from ISO string if available
        if 'created_at' in data:
            config.created_at = datetime.fromisoformat(data['created_at'])
        
        # Set all other attributes
        for key, value in data.items():
            if hasattr(config, key) and key not in ['trade_id', 'created_at']:
