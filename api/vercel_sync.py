    ...
    def sync_user_on_request(self, user_id: str, force: bool = False) -> Dict:
        ...
        try:
            ...
            with self.app.app_context():
                ...
                if (
                    user_creds
                    and user_creds.exchange_name == "toobit"
                    and user_creds.testnet_mode
                    and user_creds.has_credentials()
                ):
                    user_creds.testnet_mode = False
                    self.db.session.commit()
                    logging.info(
                        f"Disabled testnet mode for Toobit user {user_id} in Vercel environment"
                    )

                # Use centralized trading mode detection
                from .app import determine_trading_mode
    ...
    def _sync_user_positions(self, user_creds: UserCredentials) -> Dict:
        ...
        try:
            ...
            for trade in local_trades:
                result = self._sync_individual_trade(
                    trade, exchange_positions, exchange_orders
                )
                if result["updated"]:
    ...
    def _sync_individual_trade(
        ...
        try:
            ...
            if not exchange_position or float(exchange_position.get("size", 0)) == 0:
                ...
                if (
                    abs(current_price - trade.current_price) > 0.01
                    or abs(unrealized_pnl - trade.unrealized_pnl) > 0.01
                ):
            ...
            return {"updated": updated, "closed": closed, "trade_id": trade.trade_id}

        except Exception as e:
            logging.error(f"Error syncing trade {trade.trade_id}: {e}")
            return {"updated": False, "closed": False, "error": str(e)}
