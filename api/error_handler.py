...
class TradingError:
    ...
    def __init__(
        ...
        self.error_code = (
            error_code or f"{category.value}_{int(datetime.utcnow().timestamp())}"
        )
...
class ErrorClassifier:
        ...
        return {
            ...
            "api_key_invalid": {
                "patterns": [
                    ...
                    "api key not found",
                    "unauthorized",
                    f"{ErrorConfig.HTTP_UNAUTHORIZED}",
                ],
            ...
            "api_rate_limit": {
                "patterns": [
                    ...
                    "too many requests",
                    "quota exceeded",
                    f"{ErrorConfig.HTTP_RATE_LIMITED}",
                ],
            ...
            "api_server_error": {
                "patterns": [
                    ...
                    f"{ErrorConfig.HTTP_INTERNAL_ERROR}",
                    "service unavailable",
                    f"{ErrorConfig.HTTP_SERVICE_UNAVAILABLE}",
                ],
    ...
    def classify_error(self, error: Exception, context: str = "") -> TradingError:
        ...
        if "circuit breaker" in error_message and "open" in error_message:
            return self._create_trading_error(
                "circuit_breaker_open", error, error_message
            )
    ...
    def _create_trading_error(
        self, error_type: str, original_error: Exception, technical_message: str
    ) -> TradingError:
        """Create a TradingError from error type"""
        pattern_config = self.error_patterns.get(error_type, {})
        message_config = self.user_message_templates.get(
            error_type, self.user_message_templates["generic_error"]
        )
    ...
    def handle_api_error(
        self, response_data: dict, status_code: Optional[int] = None
    ) -> TradingError:
        ...
        error_msg = response_data.get(
            "msg", response_data.get("message", "Unknown API error")
        )
        error_code = response_data.get("code", status_code)

        # Specific API error handling
        if (
            status_code == ErrorConfig.HTTP_UNAUTHORIZED
            or "unauthorized" in error_msg.lower()
        ):
            return self._create_trading_error(
                "api_key_invalid", Exception(error_msg), error_msg
            )
        elif (
            status_code == ErrorConfig.HTTP_RATE_LIMITED
            or "rate limit" in error_msg.lower()
        ):
            return self._create_trading_error(
                "api_rate_limit", Exception(error_msg), error_msg
            )
        elif status_code and status_code >= ErrorConfig.HTTP_SERVER_ERROR_MIN:
            return self._create_trading_error(
                "api_server_error", Exception(error_msg), error_msg
            )
...
def handle_error(error: Exception, context: str = "", log_error: bool = True) -> dict:
    ...
    if log_error:
        logging.error(
            f"[{classified_error.category.value}] {classified_error.technical_message}"
        )
        if context:
            logging.error(f"Context: {context}")
        if classified_error.original_exception:
            logging.exception(
                "Original exception:", exc_info=classified_error.original_exception
            )
...
def handle_api_error(
    response_data: dict, status_code: Optional[int] = None, context: str = ""
) -> dict:
    """
...
def create_validation_error(
    field: str, value: Any = None, expected: Optional[str] = None
) -> dict:
    """
