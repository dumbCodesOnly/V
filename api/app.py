...
def _format_risk_assessment(margin_data):
    ...
    if margin_data["total_margin"] > 0:
        margin_ratio = (
            margin_data["total_margin"] / margin_data["account_balance"] * 100
        )
...
def _format_active_positions(active_positions):
    ...
    if active_positions:
        ...
        response += (
            f"Count: {len(active_positions)} | Total Value: ${total_value:,.2f}\n"
        )
        response += "-" * 35 + "\n"
        ...
        for config in active_positions:
            if config.symbol and config.amount:
                ...
                response += (
                    f"   Amount: ${config.amount:,.2f} | Leverage: {config.leverage}x\n"
                )
...
def _format_portfolio_statistics(user_trades, active_positions, configured_positions):
    ...
    if active_positions:
        ...
        for config in active_positions:
            if config.symbol:
                symbol_breakdown[config.symbol] = (
                    symbol_breakdown.get(config.symbol, 0) + 1
                )

        if len(symbol_breakdown) > 1:
            response += "Symbol Distribution: "
            response += " | ".join(
                [f"{sym}({count})" for sym, count in sorted(symbol_breakdown.items())]
            )
            response += "\n"
...
def _handle_portfolio_overview(chat_id):
    ...
    active_positions = [
        config for config in user_trades.values() if config.status == "active"
    ]
    configured_positions = [
        config for config in user_trades.values() if config.status == "configured"
    ]
    closed_positions = [
        config for config in user_trades.values() if config.status == "stopped"
    ]
    ...
    response += _format_portfolio_statistics(
...
def _format_position_analysis_section(user_trades, margin_data):
    ...
    long_positions = [
        c for c in user_trades.values() if c.side == "long" and c.status == "active"
    ]
    short_positions = [
        c for c in user_trades.values() if c.side == "short" and c.status == "active"
    ]
...
def handle_execute_trade(chat_id, user):
    ...
    logging.info(
        f"Executing trade: entry_type={config.entry_type}, entry_price={config.entry_price}"
    )
    ...
    if chat_id in user_selected_trade:
        ...
        if chat_id in user_trade_configs and trade_id in user_trade_configs[chat_id]:
            ...
            if config.status in ["active", "pending"]:
                logging.info(
                    f"Updated stop loss for active trade {trade_id}: {sl_percent}%"
                )
...
def handle_set_breakeven(chat_id, mode):
    ...
    if chat_id in user_selected_trade:
        ...
        if chat_id in user_trade_configs and trade_id in user_trade_configs[chat_id]:
            ...
            if config.status in ["active", "pending"]:
                logging.info(
                    f"Updated break-even setting for active trade {trade_id}: {mode_map.get(mode, 'After TP1')}"
                )
...
def handle_set_tp_percent(chat_id, tp_level, tp_percent):
    ...
    if chat_id in user_selected_trade:
        ...
        if chat_id in user_trade_configs and trade_id in user_trade_configs[chat_id]:
            ...
            if config.status in ["active", "pending"]:
                logging.info(
                    f"Updated TP{tp_level} for active trade {trade_id}: {tp_percent}%"
                )
