...
class CircuitBreaker:
    ...
    def _on_success(self, execution_time: float):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.success_count >= self.success_threshold:
                ...
                self.failure_count = 0
                logging.info(
                    f"Circuit breaker '{self.name}' recovered - back to CLOSED state"
                )
    ...
    def _on_failure(self, exception: Exception):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            self.success_count = 0
            logging.warning(
                f"Circuit breaker '{self.name}' recovery failed - back to OPEN state"
            )
        elif self.state == CircuitState.CLOSED:
            if self.failure_count >= self.failure_threshold:
                self._change_state(CircuitState.OPEN)
                logging.error(
                    f"Circuit breaker '{self.name}' OPENED - {self.failure_count} failures exceeded threshold"
                )
    ...
    def _change_state(self, new_state: CircuitState):
        ...
        if len(self.state_changes) > CircuitBreakerConfig.MAX_STATE_CHANGES:
            self.state_changes = self.state_changes[
                -CircuitBreakerConfig.MAX_STATE_CHANGES :
            ]
            ...
            return {
                ...
                * 100,
                ...
                "state_changes": self.state_changes[
                    -CircuitBreakerConfig.LAST_STATE_CHANGES_DISPLAY :
                ],  # Last state changes
                "configuration": {
    ...
    def force_open(self):
        ...
        with self._lock:
            ...
            self.last_failure_time = time.time()
            logging.warning(
                f"Circuit breaker '{self.name}' manually forced from {old_state.value} to OPEN"
            )
